...
# where n =20
N = 20
# declare population so 500 in this case
P = 200
...
for l in range (1):
    ...
    for x in range (0, P): # looping this * 50
        ...
        newind.gene = tempgene.copy()
        population.append(newind)
        newind.fitness = newerFitnessFinder(newind)

    #?
    for i in range(0,P):
        population[i].fitness = newerFitnessFinder(population[i])
    ...
    with open('Assignment Folder/AIAlg1rec.txt','a') as f:
        f.writelines("\nNUMBER OF RUNS:")
        f.writelines(str(l))
        f.writelines("\npopulation:")
        f.writelines(str(P))
...
plt.plot(Bestfitness[10:200], label = "Fitness Per Generation")
plt.plot(mean_fitness[10:200], label = "Average Fitness")
plt.legend()
plt.show()
