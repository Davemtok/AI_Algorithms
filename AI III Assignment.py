...
def newerFitnessFinder ( ind ):
    ...
    tot3 = 0
    for i in range(N):
        tot += pow(ind.gene[i], 2)
    for i in range(N):
        tot2 += pow(0.5*i*ind.gene[i],2)
    for i in range(N):
        tot3 += pow(0.5*i*ind.gene[i],4)
    tot = tot1 + tot2 + tot3
    return tot
...
with open('AIAlg2rec.txt','a') as f:
...
# try and get a value that is way below the point that its broken at
